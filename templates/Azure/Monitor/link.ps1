...
if (!($privateLinkScope)) {
  ...
  Write-Host "Updateing PrivateLinkScope Tags"
  Update-AzTag -ResourceId $privateLinkScope.Id -Tag $tags

  Write-Host "Updateing PrivateEndpoint Tags"
  $privateEndpoint = Get-AzPrivateEndpoint -ResourceGroupName $resourceGroupName -Name $privateEndpointName
  Update-AzTag -ResourceId $privateEndpoint.Id -Tag $tags
  Update-AzTag -ResourceId $privateEndpoint.NetworkInterfaces.Id -Tag $tags

  foreach ($_ in $dnsZones) {
    Write-Host "Updateing $_ DNS Zone Tags"
    $zone = Get-AzPrivateDnsZone -ResourceGroupName $resourceGroupName -Name $_
    Update-AzTag -ResourceId $zone.ResourceId -Tag $tags
  }
}